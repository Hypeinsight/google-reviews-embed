...
export async function getDashboardStats(
  ...
  try {
    ...
    const feedbackResult = await query(
      'SELECT COUNT(*) as count FROM feedback WHERE tenant_id = $1',
      [user.tenantId]
    );
    ...
    const monthFeedbackResult = await query(
      'SELECT COUNT(*) as count FROM feedback WHERE tenant_id = $1 AND created_at >= $2',
      [user.tenantId, monthStart]
    );
    ...
    const ratingResult = await query(
      'SELECT AVG(rating) as avg_rating FROM feedback WHERE tenant_id = $1 AND rating IS NOT NULL',
      [user.tenantId]
    );
...
export async function updateSettings(
  ...
  try {
    ...
    if (notificationEmail !== undefined) {
      await query(
        'UPDATE tenants SET settings = jsonb_set(COALESCE(settings, \'{}\'::jsonb), \'{notificationEmail}\', $1::jsonb) WHERE id = $2',
        [JSON.stringify(notificationEmail), user.tenantId]
      );
    }
    ...
    if (buttonText !== undefined || buttonColor !== undefined) {
      ...
      if (buttonText !== undefined) siteUpdates.buttonText = buttonText;
      if (buttonColor !== undefined) siteUpdates.buttonColor = buttonColor;

      await query(
        'UPDATE sites SET settings = settings || $1::jsonb WHERE tenant_id = $2',
        [JSON.stringify(siteUpdates), user.tenantId]
      );
    }
