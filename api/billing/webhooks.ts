...
async function handleSubscriptionUpdated(subscription: Stripe.Subscription) {
  ...
  const tierResult = await query(
    'SELECT id FROM pricing_tiers WHERE stripe_price_id = $1',
    [priceId]
  );
  ...
  await query(
    'UPDATE tenants SET subscription_status = $1, stripe_customer_id = $2 WHERE id = $3',
    [subscription.status, customerId, finalTenantId]
  );
