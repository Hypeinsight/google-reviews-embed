 ...
 * Get all team users
 */
export async function getTeamUsers(req: Request, res: Response) {
  try {
    const result = await query(
      'SELECT id, email, name, role, created_at, last_login, active FROM team_users ORDER BY created_at DESC'
    );
...
export async function createTeamUser(req: Request, res: Response) {
  try {
    ...
    const existing = await query('SELECT id FROM team_users WHERE email = $1', [email]);
    ...
    const result = await query(
      'INSERT INTO team_users (email, password_hash, name, role) VALUES ($1, $2, $3, $4) RETURNING id, email, name, role, created_at',
      [email, passwordHash, name, role || 'team_member']
    );
...
export async function updateTeamUser(req: Request, res: Response) {
  try {
    ...
    const result = await query(
      'UPDATE team_users SET email = $1, name = $2, role = $3, active = $4 WHERE id = $5 RETURNING id, email, name, role, active',
      [email, name, role || 'team_member', active !== undefined ? active : true, id]
    );
...
export async function deleteTeamUser(req: Request, res: Response) {
  try {
    ...
    const result = await query('DELETE FROM team_users WHERE id = $1 RETURNING id', [id]);
...
export async function resetPassword(req: Request, res: Response) {
  try {
    ...
    const result = await query(
      'UPDATE team_users SET password_hash = $1 WHERE id = $2 RETURNING id',
      [passwordHash, id]
    );
